function script() {
  ...
  if (isValid(cantidad)) {
    if (from != to) {
      ...
      if (cantidad < 0) {
        screen.innerHTML = `<h3 id="screen__text">No puedes convertir<br/>numeros negativos</h3>`;
      } else {
        screen.innerHTML = `<h3 id="screen__text">${cantidad} ${from} en ${to} son ${result} ${to}</h3>`;
      }
    } else {
      screen.innerHTML = `<h3 id="screen__text">No puedes convertir<br/>una moneda al mismo tipo</h3>`;
    }
  } else {
    screen.innerHTML = `<h3 id="screen__text">Introduce un valor a convertir.</h3>`;
  }
}
